...
function getPost79YearData($pdo, $year, $language ){
    ...
    $sql = $sql . " SUM(IF(P = 'OV',  SUMMA, 0)) AS 'OV', ";
    $sql = $sql . " SUM(IF(P = 'PU',  SUMMA, 0)) AS 'PU', ";
    $sql = $sql . " SUM(IF(P = 'FA',  SUMMA, 0)) + SUM(IF(P = 'FB',  SUMMA, 0)) + SUM(IF(P = 'FC',  SUMMA, 0)) + SUM(IF(P = 'OV',  SUMMA, 0)) + SUM(IF(P = 'PU',  SUMMA, 0)) AS 'TOT' ";
...
function getStrackData($pdo, $strYmdDate, $language){
    ...
    if ( $dateYear === $currentYear ){
        $data = getOngoingYearStrackData($pdo, $strYmdDate, $language);
    } else {
        $data = getCompleteYearsStrackData($pdo, $strYmdDate, $language);
    }
...
function getPost79YearDataForOneSpecies($pdo, $art, $language ){
    ...
    $sql = 'SELECT svnamn AS NAMN,';
    if ($language == 'en'){
        $sql = 'SELECT engnamn AS NAMN,';
    }

    $sql = $sql . " YEAR(DATUM) AS YEAR, snr, " ;
    $sql = $sql . " SUM(IF(P = 'FA',  SUMMA, 0)) AS 'FA', " ;
    $sql = $sql . " SUM(IF(P = 'FB',  SUMMA, 0)) AS 'FB', ";
    $sql = $sql . " SUM(IF(P = 'FC',  SUMMA, 0)) AS 'FC', ";
    $sql = $sql . " SUM(IF((P = 'ÖV' || P = 'OV') ,  SUMMA, 0)) AS 'OV', ";
    $sql = $sql . " SUM(IF(P = 'PU',  SUMMA, 0)) AS 'PU', ";
    $sql = $sql . " SUM(IF(P = 'FA',  SUMMA, 0)) + SUM(IF(P = 'FB',  SUMMA, 0)) + SUM(IF(P = 'FC',  SUMMA, 0)) + SUM(IF((P = 'ÖV' || P = 'OV'),  SUMMA, 0)) + SUM(IF(P = 'PU',  SUMMA, 0)) AS 'TOT' ";
    $sql = $sql . " FROM `v2v_rmdagsumWithNames` ";
    $sql = $sql . " WHERE ART = '" . $art . "'";
    $sql = $sql . " GROUP BY YEAR(DATUM)";
    $sql = $sql . " ORDER BY YEAR(DATUM) DESC";
