...
// Variabler för skickade artikel-nr, varukategori och språk
...
$sql_artikel = "select * from varor WHERE artnr='$modell'";
...
while ($row = mysqli_fetch_assoc($query_artikel)) {
    if ($sprak == 'sve') {
        $rad_1 = '<p><b>' . $row['artikel'] . '<br> Pris: ' . number_format($row['pris'], 0, '', ' ') . ' kr.</b></p>';
        $spectext = $row['spectext_s'] . '<br>' . $row['extra_s'];
        $backtext = 'Stäng fönstret';
    } else {
        $rad_1 = '<p><b>' . $row['item'] . '<br> Price: SEK ' . number_format($row['pris'], 2, '.', ',') . '</b></p>';
...
//tabell som visar specdata
 ...
 <tr><td colspan="3">';
                ...
                echo $spec_1 . '</td><td>';
                echo $spec_2 . '</td><td>';
                echo $spec_3 . '</td></tr><tr><td>';
                echo $spec_4 . '</td><td>';
                echo $spec_5 . '</td><td>';
                echo $spec_6 . '</td></tr><tr><td>';
                echo $spec_7 . '</td><td>';
                echo $spec_8;
            }
            ?>
