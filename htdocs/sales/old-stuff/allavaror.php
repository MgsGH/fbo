...
require_once $path . "/incl_filer/connect_new.php";
...
while ($row = mysqli_fetch_assoc($query_kat)) {
    if ($lan=='sve') {
        $headline=utf8_encode($row['kat_s']);
        $ingress=utf8_encode($row['ingr_s']);
        $bestinfo='<b>Beställning sker genom att kryssa i rutan och ange antal vid resp. artikel.<br> Klicka sedan på "Lägg till beställning" längst ned på sidan.</b>';
        ...
        $headline=$row['kat_e'];
        $ingress=$row['ingr_e'];
        $bestinfo='<b>Order by marking the checkbox and fill in quantity for each item.<br> Then, click "Add order" at the bottom of the page.</b>';
    ...
    if ($row['websajt']=='') {
        ...
        $upper_img='<a target="_blank" href="http://'.$row['websajt'].'"> <img alt="" src="bilder/'.$imgmap.'/'.$katimg.'"></a>';
    }
}
?>
<!--
...
<div width="760px" margin="auto">
    <form style="margin-bottom:0; margin-top:0" name="urval" method="POST" action="<?php $_SERVER['PHP_SELF'] ?>">
        <table width="730" class="alt_tab_bg" cellspacing="0" cellpadding="5" border="0" align="center">
          ...
          onMouseOver="overlib(\''.$minibild.'\')" onMouseOut="nd()">' . $row['artikel'] .'</a>
          </td>';
                if ($lan=='sve') {
                    echo '<td width="75">'.number_format($row['pris'], 0, '', ' ').' kr. </td>';
                }
                else {
                    echo '<td width="75">'.number_format($row['pris'], 2, '.', ',').'</td>';
                }
                echo ' 
